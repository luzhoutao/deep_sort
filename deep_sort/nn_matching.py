...
class NearestNeighborDistanceMetric(object):
    ...
    def distance(self, features, coordinates,targets,index,cross_camera,global_track,global_id, tracker_dic,world_viewer_threshold=None):
        ...
        ----------
        features : ndarray
            An NxM matrix of N features of dimensionality M.  e.g. 1x2048
        targets : List[int] -> ndarray
            A list of targets to match the given `features` against.  e.g. 1

        coordinates: ndarray
            e.g. 1x4 [[-1.2103  24.7554  43.5152 167.5927]]
        index: int (camera id)
            e.g. 0
        cross_camera: bool
            e.g. false
        global_track: list of *Track*

        global_id: list
            e.g. 1
        tracker_dic: dict e.g. {0: *Tracker*}
        ...
        """
        print(coordinates, targets, global_track, global_id, index, tracker_dic.keys(), tracker_dic[0], cross_camera)
